...
func _ready(): 
	$NextLevel/Button.connect("button_down", self, "next_level")
...
func next_level():
	if GlobalLevelConst.game_root:
		GlobalLevelConst.game_root.scene_manager.load_level(clamp(GlobalLevelConst.game_root.last_level_index + 1, 1, GlobalLevelConst.level_count))
